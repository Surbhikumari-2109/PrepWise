...
import { getAllUsers, getAllResults } from "../services/adminService";
      ...
      <div className="min-h-screen bg-slate-950 text-white p-6 md:p-10">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
            ...
            <h2 className="text-5xl font-bold mt-3">{users.length}</h2>
          </div>

          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
            <p className="text-slate-400 text-sm">Total Results</p>

            <h2 className="text-5xl font-bold mt-3">{results.length}</h2>
          </div>

          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
            <p className="text-slate-400 text-sm">Subjects Covered</p>

            <h2 className="text-5xl font-bold mt-3">{uniqueSubjects}</h2>
          </div>
        </div>
              ...
              {results.slice(0, 10).map((result) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-slate-400 text-sm">{result.subject}</p>
                  </div>
